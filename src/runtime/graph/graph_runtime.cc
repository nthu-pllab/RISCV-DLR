...
void GraphRuntime::SetupOpExecs() {
  ...
  for (uint32_t nid = 0; nid < this->GetNumOfNodes(); ++nid) {
    ...
    /* [RISCV-DLR] keep shared_ptr data to avoid release */
...
auto fexec = [arg_ptr, pf, param, op_exe]() {
    TVMRetValue rv;
    TVMArgs targs(op_exe[param.func_name].arg_values.data(),
                  op_exe[param.func_name].arg_tcodes.data(),
                  op_exe[param.func_name].arg_values.size());
    pf.CallPacked(targs, &rv);
  };
  return {fexec, arg_ptr};

  arg_ptr->arg_values = op_exe[param.func_name].arg_values;
  arg_ptr->arg_tcodes = op_exe[param.func_name].arg_tcodes;
  int num = op_exe[param.func_name].arg_values.size();
  auto fexec = [arg_ptr, pf, num]() {
    TVMRetValue rv;
